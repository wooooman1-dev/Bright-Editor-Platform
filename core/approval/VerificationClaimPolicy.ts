...
export function evaluateVerificationClaim(spec: VerificationClaimSpec, result: Omit<VerificationClaimResult, "status" | "independentInstitutionCount" | "authoritativeInstitutionCount" | "primarySourceFound">): VerificationClaimResult {
  ...
  const status = result.unresolvedConflict ? "conflicted" : !result.freshnessPassed && !freshnessUnknown ? "stale" : highRisk && (independentInstitutionCount < 3 || authoritativeInstitutionCount < 2 || !primarySourceFound) ? "insufficient" : result.normalizedValue ? "verified" : "planned";
