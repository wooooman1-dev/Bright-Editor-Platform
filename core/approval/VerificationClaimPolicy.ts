...
export function evaluateVerificationClaim(spec: VerificationClaimSpec, result: Omit<VerificationClaimResult, "status" | "independentInstitutionCount" | "authoritativeInstitutionCount" | "primarySourceFound">): VerificationClaimResult {
  ...
  const freshnessUnknown = result.diagnostics.some((diagnostic) => diagnostic === "freshness_unknown");
  const freshnessStale = result.sourceAssessments.some((source) => source.freshnessStatus === "stale");
  const status = result.unresolvedConflict ? "conflicted" : freshnessStale ? "stale" : highRisk && (independentInstitutionCount < 3 || authoritativeInstitutionCount < 2 || !primarySourceFound) ? "insufficient" : result.normalizedValue ? "verified" : freshnessUnknown ? "insufficient" : "planned";
