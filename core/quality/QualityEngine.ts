...
function evaluate(s: Signals): QualityDimensionResult[] {
  ...
  const internalLinks = s.buttons.filter((item) => item.targetUrl.startsWith("/"));
  ...
  return [
    dimension("searchIntent", s.context.searchIntent ? (s.text.toLowerCase().includes(s.context.searchIntent.toLowerCase()) ? 88 : 68) : 0,
      s.context.searchIntent ? [] : ["확정된 검색 의도 정보가 없어 평가할 수 없습니다."], s.context.searchIntent ? ["도입부와 주요 섹션에서 독자의 검색 목적을 직접 해결하세요."] : ["콘텐츠 기획의 검색 의도를 저장한 뒤 다시 검토하세요."], [{ signal: "confirmedSearchIntent", value: s.context.searchIntent ?? false }], !s.context.searchIntent),
    dimension("seo", s.keyword ? clamp(55 + (s.document.title.toLowerCase().includes(s.keyword) ? 20 : 0) + (s.keywordOccurrences > 0 ? 15 : 0) - (keywordDensity > 0.08 ? 35 : 0)) : 45,
      keywordDensity > 0.08 ? ["핵심 키워드가 지나치게 반복됩니다."] : s.keyword && !s.document.title.toLowerCase().includes(s.keyword) ? ["제목이 핵심 키워드를 명확히 반영하지 않습니다."] : [], ["제목과 본문에 핵심 키워드를 자연스럽게 배치하고 반복을 줄이세요."], [{ signal: "keywordOccurrences", value: s.keywordOccurrences }, { signal: "keywordDensity", value: Number(keywordDensity.toFixed(3)) }]),
    ...
    dimension("structure", clamp(20 + Math.min(55, s.headings.length * 12) + (intro.length >= 80 ? 15 : 0) - s.duplicateHeadingCount * 15 - s.emptyHeadings * 20 - (invalidHeadingOrder ? 20 : 0)),
    ...
    dimension("usefulness", clamp(depthScore - (s.placeholders ? 40 : 0) - (externalClaims && !hasCitation ? 25 : 0)),
      [...(s.placeholders ? ["placeholder 또는 작성 지시 문구가 남아 있습니다."] : []), ...(externalClaims && !hasCitation ? ["수치나 연구 주장을 뒷받침하는 출처를 확인할 수 없습니다."] : [])], ["독자가 실행할 수 있는 구체적 설명과 검증 가능한 근거를 추가하세요."], [{ signal: "placeholderDetected", value: s.placeholders }, { signal: "unsupportedClaimSignal", value: externalClaims && !hasCitation }], externalClaims && !hasCitation),
    dimension("htmlQuality", clamp(90 - s.emptyHeadings * 30 - s.duplicateHeadingCount * 15 - (invalidHeadingOrder ? 25 : 0) - (s.document.blocks.length ? 0 : 90)),
    ...
    dimension("imageStrategy", s.images.length ? clamp(55 + Math.min(30, s.images.length * 10) + (s.images.every((item) => item.alt.trim()) ? 15 : 0) - (s.images.every((item) => !item.source) ? 25 : 0)) : 10,
      s.images.length ? (s.images.every((item) => !item.source) ? ["이미지 계획 메타데이터만 있고 실제 문서 이미지가 준비되지 않았습니다."] : []) : ["본문에 반영된 이미지 전략이 없습니다."], ["관련 이미지 블록을 실제 소스와 설명 텍스트로 본문에 배치하세요."], [{ signal: "imageBlocks", value: s.images.length }, { signal: "resolvedImages", value: s.images.filter((item) => Boolean(item.source)).length }]),
    dimension("internalLinks", internalLinks.length ? Math.min(100, 65 + internalLinks.length * 15) : 10,
      internalLinks.length ? [] : ["본문에 실제 배치된 내부 링크가 없습니다."], ["독자의 다음 행동과 관련된 기존 콘텐츠 링크를 본문에 배치하세요."], [{ signal: "placedInternalLinks", value: internalLinks.length }]),
    dimension("cta", s.buttons.length ? clamp(55 + Math.min(25, s.buttons.length * 15) + (s.buttons.some((item) => item.label.trim().length >= 4) ? 15 : 0)) : 10,
      s.buttons.length ? [] : ["독자가 실행할 수 있는 실제 CTA가 없습니다."], ["글의 목표와 연결되는 구체적인 CTA 버튼을 추가하세요."], [{ signal: "ctaBlocks", value: s.buttons.length }]),
  ];
}

function dimension(category: QualityCategory, score: number, reasons: string[], tasks: string[], evidence: QualityEvidence[], notEvaluated = false): QualityDimensionResult {
  const normalized = clamp(Math.round(score));
  const status: QualityDimensionStatus = notEvaluated ? "blocked" : normalized >= 85 ? "ready" : "needs_improvement";
  return Object.freeze({ category, score: notEvaluated ? 0 : normalized, status, evaluation: notEvaluated ? "not_evaluated" : "evaluated", reasons: Object.freeze(reasons.length ? reasons : normalized >= 85 ? [] : ["현재 측정값이 게시 준비 기준에 미치지 못합니다."]), tasks: Object.freeze(status === "ready" ? [] : tasks), evidence: Object.freeze(evidence) });
}
