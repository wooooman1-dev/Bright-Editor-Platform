...
export function SettingsDataSources({ projects, workspaceId }: { projects: readonly Readonly<{ id: string; name: string }>[]; workspaceId: string }) {
  ...
  const beginNewConnection = useCallback((nextProvider: DataSourceProvider) => {
  ...
  const refresh = useCallback(async (preferredConnectionId?: string) => {
    ...
    const result = await response.json() as { connections?: PublicDataSourceConnection[]; projectReferences?: ProjectReference[]; googleOAuth?: { configured?: boolean }; error?: string };
    if (!response.ok) throw new Error(result.error ?? "Data Sources를 불러오지 못했습니다.");
    const nextConnections = result.connections ?? [];
    setConnections(nextConnections);
    setReferences(result.projectReferences ?? []);
  ...
  const save = async () => {
    ...
    const result = await action({ action: "save-connection", provider, displayName, resourceConfiguration: configuration, credentials, enabled: true, ...(existing ? { connectionId: existing.id, connectionVersion: existing.version } : {}) }) as { connection?: PublicDataSourceConnection };
    setAccessToken(""); setClientId(""); setClientSecret("");
    if (result.connection) editConnection(result.connection);
    setNotice(existing ? "연결 구성을 업데이트했습니다." : "새 데이터 소스 연결을 저장했습니다.");
  };

  const reuseGoogleConnection = async (source: PublicDataSourceConnection) => {
    const result = await action({ action: "create-google-resource-connection", sourceConnectionId: source.id, displayName }) as { connection?: PublicDataSourceConnection };
    if (!result.connection) throw new Error("새 Google 리소스 연결을 만들지 못했습니다.");
    editConnection(result.connection);
    setNotice(`${source.displayName}의 Google 인증을 안전하게 재사용했습니다. 사용할 리소스를 선택해 저장해 주세요.`);
  };

  const startGoogleOAuth = (targetProvider: DataSourceProvider, connectionId?: string) => {
    const query = new URLSearchParams({ workspaceId, provider: targetProvider, returnTo: window.location.pathname, ...(connectionId ? { connectionId } : {}) });
  ...
  const deleteDataSource = async (connection: PublicDataSourceConnection) => {
    ...
    try {
      ...
      if (editingConnectionId === connection.id) beginNewConnection(connection.provider);
      await refresh();
  ...
  return <div className="space-y-5">
    <section className="rounded-[20px] border border-black/6 bg-white p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-[#77777f]">한 Provider에 여러 사이트·채널·키워드 세트를 연결하고 Project별로 필요한 Evidence만 배정합니다.</p>
        </div>
        <button className="rounded-xl bg-[#ff6b6b] px-5 py-3 text-sm font-semibold text-white" disabled={!selectedProvider.enabled || busy} onClick={() => beginNewConnection(provider)} type="button">새 연결 추가</button>
      </div>

      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{providers.map((item) => {
        const count = connections.filter((value) => value.provider === item.provider).length;
        return <button className={`rounded-xl border p-4 text-left ${provider === item.provider ? "border-[#ff6b6b] bg-[#fff7f7]" : ""}`} disabled={!item.enabled} key={item.provider} onClick={() => beginNewConnection(item.provider)} type="button">
          ...
          <span className="mt-2 block text-xs leading-5 text-[#77777f]">{item.description}</span>
          {!item.enabled ? <span className="mt-2 block text-xs font-semibold text-amber-800">구성 필요</span> : null}
        </button>;
      })}</div>

      {selectedProvider.enabled ? <div className="mt-6 scroll-mt-6 rounded-2xl border bg-[#fcfcfd] p-4 sm:p-5" ref={editorAnchor}>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          {editingConnection ? <button className="rounded-lg border px-3 py-2 text-sm font-semibold" onClick={() => beginNewConnection(provider)} type="button">새 연결로 전환</button> : null}
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {!isGoogleOAuthProvider(provider) || hasGoogleOAuthCredential ? <div className="sm:col-span-2">
            <button className="rounded-xl bg-[#ff6b6b] px-5 py-3 text-sm font-semibold text-white disabled:opacity-50" disabled={busy} onClick={() => void save().catch(handleError)} type="button">{editingConnectionId ? "연결 구성 업데이트" : "새 연결 저장"}</button>
            {editingConnectionId ? <button className="ml-2 rounded-xl border px-5 py-3 text-sm font-semibold" onClick={() => beginNewConnection(provider)} type="button">편집 취소</button> : null}
    ...
    <section className="rounded-[20px] border border-black/6 bg-white p-5 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div><h2 className="text-lg font-semibold">Project별 데이터 소스 배정</h2><p className="mt-1 text-sm text-[#77777f]">Workspace 연결 중 이 Project의 Opportunity Planning에서 사용할 연결만 선택합니다.</p></div>
        {projects.length ? <label className="text-sm font-semibold">Project<select className="ml-2 rounded-lg border px-3 py-2 font-normal" onChange={(event) => setProjectId(event.target.value)} value={projectId}>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label> : null}
      </div>
      <div className="mt-5 space-y-3">{connections.length ? connections.map((connection) => {
        ...
        return <article aria-current={editingConnectionId === connection.id ? "true" : undefined} className={`rounded-xl border p-4 ${editingConnectionId === connection.id ? "border-[#ff6b6b] bg-[#fffafa]" : ""}`} key={connection.id}>
          <div className="flex flex-wrap justify-between gap-3">
            <div>
              <p className="font-semibold">{providerLabel(connection.provider)} · {connection.displayName}</p>
              <p className="mt-1 text-sm text-[#77777f]">{statusLabel(connection.status, reconnectRequired)} · freshness {freshnessLabel(connection.freshness)}{referenced ? " · 이 Project에 배정됨" : ""}</p>
          ...
          {projectId ? <label className="mt-4 flex gap-2 border-t pt-4 text-sm"><input checked={referenced} onChange={(event) => void action({ action: "set-project-reference", projectId, connectionId: connection.id, enabled: event.target.checked }).catch((error) => setNotice(message(error)))} type="checkbox" />이 Project의 Opportunity Planning에서 사용</label> : null}
        </article>;
      }) : <p className="rounded-xl bg-[#f8f8fa] p-4 text-sm text-[#77777f]">구성된 Data Source가 없습니다. 위의 ‘새 연결 추가’에서 첫 연결을 만들어 주세요.</p>}</div>
    </section>
...
function oauthOutcomeMessage(outcome: string, code: string | null, provider: string) {
  ...
  if (outcome === "resourceRequired") return `Google 계정을 연결했습니다. 사용할 ${provider === "youtubeAnalytics" ? "YouTube 채널" : "Search Console 사이트 속성"}을 선택해 주세요.`;
...
type OAuthReturn = Readonly<{ outcome: string; connectionId: string; provider: string; message: string }>;
const emptyOAuthReturn: OAuthReturn = Object.freeze({ outcome: "", connectionId: "", provider: "", message: "" });
...
export function parseOAuthReturn(search: string): OAuthReturn {
  ...
  return Object.freeze({ outcome, connectionId, provider, message: outcome ? oauthOutcomeMessage(outcome, query.get("oauthCode"), provider) : "" });
}
...
function removeOAuthReturnQuery() {
  ...
  query.delete("dataSourceOAuth"); query.delete("connectionId"); query.delete("oauthCode"); query.delete("dataSourceProvider");
