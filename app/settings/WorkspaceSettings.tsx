  ...
  return <main className="min-h-screen bg-[#f8f8fa] text-[#19191b]">
    ...
    <div className="mx-auto grid max-w-7xl gap-6 px-5 py-8 sm:px-8 lg:grid-cols-[220px_1fr]">
      ...
      <div><header className="mb-6"><p className="text-xs font-semibold tracking-[0.14em] text-[#ff6b6b] uppercase">Workspace Settings</p><h1 className="mt-2 text-3xl font-semibold">{sections.find((item) => item.id === section)?.label}</h1></header>{error ? <p className="mb-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}
        ...
        {section === "publishing" ? <PublishingSettings busy={busy} onSave={(value, wordpressSchedulePublicPublish) => action({ action: "save-publishing", sequentialDraftSave: value, wordpressSchedulePublicPublish })} snapshot={snapshot} /> : null}
...
function AISettings({ busy, onCheck, snapshot }: { busy: boolean; onCheck: () => Promise<unknown>; snapshot: SettingsSnapshot }) {
  return <Card><div className="flex justify-between gap-4"><div><h2 className="text-lg font-semibold">OpenAI</h2><p className="mt-1 text-sm text-[#77777f]">역할별 모델 정책</p></div><Status status={snapshot.ai.status} /></div><dl className="mt-5 grid gap-3 sm:grid-cols-2"><Metric label="기획·원고 생성 모델" value={snapshot.ai.generationModel} /><Metric label="최종 품질 검토·편집 모델" value={snapshot.ai.reviewModel} /></dl><p className="mt-5 rounded-xl bg-[#f8f8fa] p-4 text-sm">{snapshot.ai.message}</p><p className="mt-3 text-xs leading-5 text-[#77777f]">원고 생성은 1회, 최종 품질 검토·편집은 1회만 호출합니다. 95점 승인 기준을 통과한 원고만 편집기에 표시합니다.</p><button className="mt-5 rounded-xl border px-4 py-2.5 text-sm font-semibold" disabled={busy} onClick={() => void onCheck()} type="button">설정 상태 확인</button></Card>;
}

function PublishingSettings({ busy, onSave, snapshot }: { busy: boolean; onSave: (value: boolean, wordpressSchedulePublicPublish: boolean) => Promise<unknown>; snapshot: SettingsSnapshot }) {
  const [sequential, setSequential] = useState(snapshot.settings.publishing.sequentialDraftSave);
  const [schedulePublic, setSchedulePublic] = useState(snapshot.settings.publishing.wordpressSchedulePublicPublish === true);
  return <Card><h2 className="text-lg font-semibold">기본 발행 정책</h2><div className="mt-5 space-y-3"><Policy label="검토 후 진행" checked disabled /><Policy label="임시저장만 허용" checked disabled /><Policy label="즉시 공개 발행" checked={false} disabled /><Policy label="계정별 순차 임시저장" checked={sequential} onChange={setSequential} /><Policy label="WordPress 예약 공개 발행 허용" checked={schedulePublic} onChange={setSchedulePublic} /></div><p className="mt-3 rounded-xl bg-[#f8f8fa] p-4 text-xs leading-5 text-[#77777f]">예약 공개 발행을 끄면 WordPress 예약은 초안 상태로만 등록됩니다. 켜면 현재 Revision의 품질·승인준비·권한 검사를 통과한 글을 지정한 시각에 자동 공개할 수 있습니다. 즉시 공개 발행 정책은 계속 꺼져 있습니다.</p><h3 className="mt-6 font-semibold">플랫폼 준비 상태</h3><dl className="mt-3 grid gap-3 sm:grid-cols-2">{snapshot.settings.enabledPlatforms.map((platform) => <Metric key={platform} label={platformLabels[platform]} value={statusLabel(snapshot.platforms[platform]!.status)} />)}</dl><button className="mt-5 rounded-xl bg-[#ff6b6b] px-5 py-3 text-sm font-semibold text-white" disabled={busy} onClick={() => void onSave(sequential, schedulePublic)} type="button">발행 설정 저장</button></Card>;
}
