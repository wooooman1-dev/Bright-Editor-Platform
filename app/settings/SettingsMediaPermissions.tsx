...
export function SettingsMediaPermissions({ connections, onRefresh, workspaceId }: { connections: readonly PublicConnection[]; onRefresh: () => Promise<void>; workspaceId: string }) {
  ...
  const draftConnections = connections.filter((connection) => connection.platform === "tistory" || connection.platform === "wordpress");

  const update = async (connection: PublicConnection, enabled: boolean) => {
    if (enabled && !window.confirm(`이 ${connection.platform === "wordpress" ? "WordPress" : "Tistory"} 계정의 Draft 저장 과정에서 로컬 이미지를 업로드하도록 허용할까요? 공개 발행 권한은 켜지지 않습니다.`)) return;
    ...
    try {
      ...
      await onRefresh();
      const platformName = connection.platform === "wordpress" ? "WordPress" : "Tistory";
      setNotice(enabled ? `${platformName} 이미지 업로드를 허용했습니다. Draft Only와 최종 확인 정책은 그대로 유지됩니다.` : `${platformName} 이미지 업로드 권한을 껐습니다. 이미지가 포함된 원고는 외부 Draft 저장 전에 중단됩니다.`);
    } catch (error) {
  ...
  return <div className="space-y-5">
    <section className="rounded-[20px] border border-black/6 bg-white p-5 sm:p-6">
      <h2 className="text-lg font-semibold">Draft 이미지 업로드 권한</h2>
      <p className="mt-2 text-sm leading-6 text-[#77777f]">기본값은 꺼짐입니다. 켜면 사용자가 최종 확인한 Draft 저장 작업 안에서만 로컬 이미지를 선택한 플랫폼에 업로드합니다. 공개 발행, 기존 글 수정, 삭제 권한은 추가되지 않습니다.</p>
      <div className="mt-5 space-y-3">
        {draftConnections.length ? draftConnections.map((connection) => {
        ...
        }) : <p className="rounded-xl border border-dashed p-4 text-sm text-[#77777f]">Tistory 또는 WordPress 계정을 먼저 연결해 주세요.</p>}
      </div>
    </section>
