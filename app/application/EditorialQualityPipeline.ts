...
Verified factual surfaces are recorded by the server, not rewritten here. Do not introduce a new externally verifiable fact — an amount, rate, date, period, eligibility rule, threshold, or legal proposition — that the current manuscript does not already state. Edit the prose around an existing factual sentence rather than paraphrasing its value away.
  ${document.metadata?.generatedFactualClaimInventory ? `Return the complete current factual inventory as top-level verificationClaimsUsed. It must contain every protected factual surface exactly once with the same claimId, origin, risk, statement, surfaceText, Evidence URL/excerpt, semantic JSON, qualifiers, and temporal requirement. Copy every field verbatim from the canonical inventory given below; never paraphrase, reformat, or reconstruct a value from memory. Do not use origin=quality_review and do not add a new factual item. If a correction would require a new fact, keep the existing supported wording or use NONE advice instead. The server rejects this candidate if the inventory is missing, changed, incomplete, or contains a new Claim.
  Canonical factual claim inventory (copy every field exactly, do not infer or paraphrase): ${JSON.stringify(document.metadata.generatedFactualClaimInventory)}` : "Do not add new externally verifiable facts during this edit."}
