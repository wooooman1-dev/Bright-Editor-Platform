...
Return no more than one source-empty image recommendation block for the entire article, and return zero when an image is not materially needed or a reusable Project image can serve the role. Prefer a representative hero image when a new image is necessary. Do not create image blocks for comparison, checklist, summary, or warning content; represent those as a Markdown table, list, checklist, or Bright HTML/SVG component instead. The one optional image purpose must be hero, inline, or infographic and must include a specific Korean ALT plus a standalone production prompt. Decide whether CTA is useful; when useful place at most two CTA button blocks in context. Never invent a URL. If no approved CTA URL is known, use an empty targetUrl. Do not create internal, monetization, or related-post links unless their real approved URLs are supplied in the editorial context; Bright Studio will place verified catalog links separately.
Paragraph values must be plain text. When a list is needed, use newline-prefixed 1. or - items inside the paragraph string. When a table is needed, put a complete GitHub-style Markdown table in one standalone paragraph string, including a header row, a separator row such as |---|---|, and at least one data row. Never return HTML tags such as <table>, <ol>, <ul>, <li>, or <p>. Return JSON only in this exact generation shape: {"title":"...","metaDescription":"...","primarySearchIntent":"...","secondaryIntent":"...","secondaryKeywords":["..."],"relatedTerms":["..."],"tags":["티스토리태그1","티스토리태그2"],"introduction":["paragraph"],"sections":[{"heading":"H2 heading","sectionType":"explanation","paragraphs":["paragraph"]}],"conclusion":["paragraph"],"images":[{"afterSection":0,"purpose":"hero","alt":"specific Korean ALT","prompt":"standalone image production prompt"}],"cta":[]}. sectionType must be one of explanation, checklist, comparison, steps, warning, faq, summary, case_example. afterSection is 0 for placement after the introduction or a 1-based section number. CTA items use {"afterSection":0,"purpose":"cta","label":"...","targetUrl":"","target":"_self"}. Do not return blocks from the generation call.`,
    };
  }
  ...
  parse(response: string, input: GenerationInput): ContentDocument {
    ...
    const costOptimized = applyGeneratedImageCostPolicy(normalized);
