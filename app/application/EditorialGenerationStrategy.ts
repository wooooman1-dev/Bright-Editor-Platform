...
import { contentLengthProfile } from "../../core/quality";
...
export class EditorialGenerationStrategy implements ContentGenerationStrategy {
  createRequest(input: GenerationInput) {
    ...
    const lengthProfile = contentLengthProfile(input.contentType, input.platform);
...
Do not return an outline, plan, writing instructions, analysis, or placeholders as article prose. For a Tistory long-form article, build the H2 structure from the topic and confirmed search intent rather than forcing a fixed section count. Use H3 only when it genuinely clarifies a subsection. The server scores paragraph prose after removing whitespace. The minimum for this content contract is ${lengthProfile.minimumCharacters.toLocaleString("ko-KR")} non-whitespace prose characters and the generation target is ${lengthProfile.targetCharacters.toLocaleString("ko-KR")} characters; never aim at the minimum boundary. Keep every section focused on its own editorial purpose and explain that purpose with sufficient depth. Include practical steps, examples, comparisons, cautions, exceptions, or alternatives only when they are useful for that section; never force all of them into every H2. Every H2 must still contain developed prose and must not be empty, repetitive, or superficial. Before returning JSON, verify that the body reaches the ${lengthProfile.targetCharacters.toLocaleString("ko-KR")} character target and never falls below ${lengthProfile.minimumCharacters.toLocaleString("ko-KR")}, that every section fulfills its intended role in answering the confirmed search intent, and that no required topic from the Content Opportunity is omitted. Start with a substantive introduction that gives the core answer and end with a natural conclusion of at least three connected sentences.
...
Quality Engine compatibility is mandatory. Do not use vague action phrases such as “일정 기간”, “잠시”, “필요한 경우”, “상황에 따라”, “적절한 때”, “충분히 쉬고”, or “며칠간” unless the same sentence immediately defines an observable condition, sequence, decision boundary, or safe time range supported by the supplied context. Where exact external numbers are not verified, use concrete non-numeric criteria such as visible changes, symptom progression, applicability conditions, exceptions, or a numbered sequence using “첫째”, “둘째”, and “셋째”; never invent medical numbers merely to sound specific. Include at least three genuinely useful practical signals across the article, such as a checklist, a worked example, a step sequence, a comparison standard, a recording method, or an action flow. State recurring cautions and core advice once in the best-fitting section instead of repeating variants of “같은 조건”, “한 번의 수치”, “기록을 남기기”, “의료진과 상담”, or “임의로 바꾸지 않기” across multiple H2 sections. If another section depends on the same principle, add new situation-specific information rather than paraphrasing the advice.
...
Reader usefulness is also a mandatory completion condition, not a vague preference. Every H2 must directly fulfill its own heading and editorial purpose, and provide new, non-duplicative information that the reader can use to understand, compare, judge, check, choose, or act. Select only the types of value that fit the section purpose: core concept or mechanism; causes and distinguishing criteria; situation-specific differences; selection or decision criteria; observable checks; steps or sequence; who the guidance applies to or does not apply to; exceptions; common mistakes; or a sensible next action. Do not force all of these into every section, but do not leave any H2 as generic explanation, restated title text, or abstract encouragement. Statements such as “꾸준한 관리가 중요합니다”, “건강한 생활 습관이 필요합니다”, “자신에게 맞는 방법을 선택해야 합니다”, “충분한 휴식과 균형 잡힌 식사가 중요합니다”, or “전문가와 상담하는 것이 좋습니다” may appear only when immediately followed by the concrete meaning, decision criterion, observable condition, application context, or next step. Each H2 must differ materially from the previous section and must not pad length by paraphrasing the same idea. When exact evidence is unavailable, keep the writing concrete through observable criteria, conditional differences, applicability, exceptions, common misunderstandings, and when professional assessment is appropriate; never retreat into empty generalities. The conclusion must not merely repeat the body: it must help the reader choose a next decision or action based on the article's main criteria. Before returning JSON, self-check every H2 by asking: does this section directly fulfill its heading and editorial purpose; what new information does the reader gain; what concrete criterion or check can the reader use when appropriate; is this section distinct from the others; and did I avoid generic advice. If any answer is missing, revise that section before returning the article.
