...
Do not return an outline, plan, writing instructions, analysis, or placeholders as article prose. For a Tistory long-form article, use five or six developed H2 sections by default. A genuinely complex topic may use seven H2 sections, but never create eight or more. Do not add sections merely to reach the length target; deepen the existing sections instead. Use H3 only when it genuinely clarifies a subsection. The server approval floor remains 4,800 non-whitespace Korean prose characters, but your generation target is 6,000–6,500 non-whitespace Korean prose characters and your pre-return safety target is at least 5,500. Aim for a substantive 400–500-character introduction, five or six H2 sections of roughly 750–950 non-whitespace prose characters each, and a 400–500-character conclusion. These are information-density budgets, not permission to repeat or pad. Each H2 must answer one materially different reader question or decision and contain at least 450 non-whitespace Korean prose characters, normally across two or three connected paragraphs. Keep every section focused on its own editorial purpose and explain that purpose with sufficient depth. Include causes, distinctions, decision criteria, examples, practical steps, cautions, exceptions, common mistakes, or alternatives only when they are useful for that section; never force all of them into every H2. Add depth by supplying new explanation, observable distinctions, decision rules, situation-specific examples, action sequence, applicability, exceptions, or next actions—not by paraphrasing earlier advice. Every H2 must be developed, non-repetitive, and professionally useful rather than superficial. Before returning JSON, verify that there are five to seven H2 sections, no H2 is shallow, the body reaches the 6,000–6,500 target or at minimum the 5,500 safety target, every section fulfills a distinct role in answering the confirmed search intent, no required topic from the Content Opportunity is omitted, repeated core advice is zero, and the article contains at least three useful observable criteria, supported time/sequence conditions, decision rules, or concrete examples without inventing medical thresholds. Start with a substantive introduction that gives the core answer and end with a natural concise conclusion.
