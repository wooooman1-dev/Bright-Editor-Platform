...
type SafeConnection = Readonly<{
  ...
  publicMetadata?: Readonly<Record<string, unknown> & { sessionStateAvailable?: boolean }>;
}>;
type PreviewMode = "desktop" | "mobile";
type EditorViewMode = "visual" | "preview" | "html";
...
export function EditorWorkspace({ content, data, project, onBack, onPersist }: { content: UserContent; data: UserData; project: UserProject; onBack: () => void; onPersist: (data: UserData) => Promise<void> }) {
  ...
  const [connections, setConnections] = useState<readonly SafeConnection[]>([]); const [connectionId, setConnectionId] = useState(content.publishingPreparation?.tistory?.publishingAccountId ?? content.publishingAccountId ?? project.strategy?.defaultPublishingAccountId ?? (content.selectedPublishingAccountIds?.length === 1 ? content.selectedPublishingAccountIds[0] : ""));
  ...
  const [categorySource, setCategorySource] = useState<"content" | undefined>(initialPreparation ? "content" : undefined);
  const [proposedCategory, setProposedCategory] = useState<Readonly<{ id: string | null; name: string | null }>>();
  ...
  const { activePlatform, tistoryEnabled, wordpressEnabled } = editorPublishingPlatformVisibility({
    enabledPlatforms: data.workspace?.settings?.enabledPlatforms,
    project,
    content,
  });
  ...
  const currentRevisionId = liveDocument ? editorialRevisionId(liveDocument) : "";
  ...
  useEffect(() => {
    ...
    void fetch(`/api/connections?workspaceId=${encodeURIComponent(project.workspaceId)}`, { cache: "no-store" })
      .then(async (response) => {
        const result = await response.json() as { connections?: SafeConnection[]; error?: string };
        if (!response.ok) throw new Error(result.error ?? "플랫폼 연결을 불러오지 못했습니다.");
        if (!active) return;
        const values = result.connections ?? [];
        setConnections(values);
        if (!tistoryEnabled) return;
        const storedIds = [
          content.publishingPreparation?.tistory?.publishingAccountId,
          content.publishingAccountId,
          ...(content.selectedPublishingAccountIds ?? []),
        ...
        const requestedConnectionId = storedIds.find((id) => id && values.some((item) => item.platform === "tistory" && item.id === id));
        const prepared = await api("/api/tistory", {
          action: "prepare",
          workspaceId: project.workspaceId,
          projectId: project.id,
          contentId: content.id,
          ...(requestedConnectionId ? { connectionId: requestedConnectionId } : {}),
        }) as { data?: UserData; connectionId?: string | null; readiness?: TistoryReadiness };
        if (!active) return;
        setConnectionId(prepared.connectionId ?? "");
        if (prepared.readiness) setReadiness(prepared.readiness);
        if (prepared.data) await onPersist(prepared.data);
      })
  ...
  useEffect(() => {
    if (!activePlatform || !content.document) return;
    let active = true;
    void Promise.resolve()
      .then(() => {
        if (active) setOperation("preview");
        return api("/api/studio", {
          action: "render-platform",
          input: {
            contentId: content.id,
            workspaceId: project.workspaceId,
      ...
      .then((result) => {
        if (active && typeof (result as { html?: unknown }).html === "string") {
          setPreviewHtml((result as { html: string }).html);
        }
      })
      .catch((error) => {
        if (active) setNotice(`미리보기를 자동 생성하지 못했습니다. ${message(error)}`);
      })
      .finally(() => {
        if (active) setOperation("idle");
      });
    return () => { active = false; };
  }, [activePlatform, content.document, content.id, project.workspaceId]);
  ...
  const commitDocument = async (document: ContentDocument, success: string, baseData: UserData = data) => {
    const next = applyCanonicalDocument(baseData, content.id, document, "manual", now());
    setSaveState("saving");
  ...
  const requestQualityImprovement = async () => {
    ...
    try {
      ...
      const feedback = "한 번의 AI 품질 편집으로 승인 기준에 도달하지 못했습니다. 현재 원고는 변경하지 않았습니다.";
      setImprovementPreview(result.document); setImprovementBaseRevision(result.basedOnRevisionId ?? ""); setImprovementBaselineQuality(result.baselineQuality); setImprovementCandidateQuality(result.quality); setImprovementDecision(result.improvement); setImprovementFeedback({ tone: "warning", message: feedback }); setNotice(feedback);
  ...
  const approveQualityImprovement = async () => {
    if (!improvementPreview || improvementCandidateQuality?.approved !== true || improvementCandidateQuality.approvalType !== "standard" || !improvementDecision?.accepted) return; setWorking(true); setOperation("applying");
    try {
  ...
  const saveTistory = async () => {
    if (!tistoryEnabled) { setNotice("Tistory is disabled in Workspace Settings."); return; }
  ...
  async function selectConnection(accountId: string) {
    setConnectionId(accountId); setCategoryId(""); setCategoryName(null); setCategorySource(undefined); setProposedCategory(undefined); setReadiness(undefined);
    if (!accountId) return;
    try {
  ...
  async function loadCategories(accountId: string) {
    ...
    try { const response = await fetch(`/api/tistory/categories?workspaceId=${encodeURIComponent(project.workspaceId)}&contentId=${encodeURIComponent(content.id)}&connectionId=${encodeURIComponent(accountId)}`, { cache: "no-store" }); const result = await response.json() as { categories?: SafeCategory[]; preparation?: NonNullable<UserContent["publishingPreparation"]>["tistory"] | null; selection?: Readonly<{ source: "content" | "project"; categoryId: string | null; categoryName: string | null }> | null; safeMessage?: string; error?: string; reconnectRequired?: boolean; stale?: boolean; failureCode?: string }; if (!response.ok) throw Object.assign(new Error(result.safeMessage ?? result.error ?? "카테고리 목록을 불러오지 못했습니다."), { reconnectRequired: result.reconnectRequired }); setCategories(result.categories ?? []); if (result.preparation) { setCategoryId(result.preparation.platformCategoryId ?? "__uncategorized__"); setCategoryName(result.preparation.platformCategoryName); setCategorySource("content"); setProposedCategory(undefined); } else { setCategoryId(""); setCategoryName(null); setCategorySource(undefined); setProposedCategory(result.selection?.source === "project" ? { id: result.selection.categoryId, name: result.selection.categoryName } : undefined); } setCategoryState(result.stale ? "stale" : result.categories?.length ? "ready" : "empty"); setCategoryMessage(result.stale ? `마지막으로 확인된 카테고리 목록을 표시합니다. ${result.preparation ? `저장된 ${result.preparation.platformCategoryName ?? "카테고리 없음"} 카테고리는 계속 적용됩니다.` : "제안값은 아직 콘텐츠에 적용되지 않았습니다."}` : ""); }
  ...
  async function selectCategory(value: string) {
    ...
    setOperation("category-save");
    setCategoryMessage("");

    try {
      const result = await api("/api/tistory/categories", {
        workspaceId: project.workspaceId,
        contentId: content.id,
        connectionId: selectedConnection.id,
        categoryId: value === "__uncategorized__" ? null : value,
      }) as {
        preparation?: NonNullable<UserContent["publishingPreparation"]>["tistory"];
        data?: UserData;
      };

      if (!result.preparation || !result.data) {
        throw new Error("카테고리를 저장하지 못했습니다. 다시 시도해 주세요.");
      }

      const preparation = result.preparation;

      setCategoryId(preparation.platformCategoryId ?? "__uncategorized__");
      setCategoryName(preparation.platformCategoryName);
      setCategorySource("content");
      setProposedCategory(undefined);

      await onPersist(result.data);

      setCategoryState(categories.length ? "ready" : "empty");
      setNotice(`카테고리 적용 완료: ${preparation.platformCategoryName ?? "카테고리 없음"} · 공개 글 후보를 자동 확인합니다.`);

      await loadPostCandidates(false, {
        categoryId: preparation.platformCategoryId,
        categoryName: preparation.platformCategoryName,
        data: result.data,
      });

      await loadReadiness(selectedConnection.id);
    } catch (error) {
      setCategoryState(
        message(error) === "재연결 필요" ? "expired" : "error",
      );
      setCategoryMessage(
        message(error) === "재연결 필요"
          ? "Tistory 로그인 세션이 만료되었습니다. 설정에서 다시 연결해 주세요."
          : "카테고리를 저장하지 못했습니다. 다시 시도해 주세요.",
      );
    } finally {
      setOperation("idle");
    }
  ...
  return <PageContainer className="py-8 sm:py-10 lg:py-12">
    ...
    <section className="mt-6 rounded-[24px] border border-black/6 bg-white p-6"><label className="block text-sm font-semibold">제목<input className="mt-2 w-full rounded-xl border px-4 py-3 text-xl disabled:opacity-60" disabled={working} onBlur={() => liveDocument && void commitDocument({ ...liveDocument, title }, "문서 제목을 저장했습니다.")} onChange={(event) => { setTitle(event.target.value); setQualityRequestState("idle"); setFinalConfirmation(false); }} value={title} /></label><ContentSeoTitleStatus currentTitle={liveDocument?.metadata?.seoTitle ?? title} disabled={working || !liveDocument?.metadata} onApply={async (seoTitle) => { if (liveDocument?.metadata) await commitDocument({ ...liveDocument, metadata: Object.freeze({ ...liveDocument.metadata, seoTitle }) }, "SEO 제목을 저장했습니다."); }} primaryKeyword={content.primaryKeyword} /><p className="mt-3 text-sm text-[#77777f]">본문은 아래 canonical 블록 편집기에서 수정합니다. 블록 순서가 Preview와 Draft HTML에 그대로 반영됩니다.</p><div className="mt-4 flex flex-wrap gap-5 rounded-xl bg-[#f8f8fa] px-4 py-3 text-sm"><span><strong>문단 수</strong> {metrics.paragraphCount}</span><span><strong>소제목 수</strong> {metrics.headingCount}</span><span><strong>예상 읽기 시간</strong> {metrics.estimatedReadingMinutes}분</span></div><details className="mt-3 text-sm text-[#66666f]"><summary className="cursor-pointer font-semibold">상세 진단·비용 지표</summary><dl className="mt-3 grid gap-2 sm:grid-cols-3"><Info label="본문 공백 제외 글자 수" value={metrics.editorialCharactersWithoutSpaces.toLocaleString()} /><Info label="부록·이미지 설명 글자 수" value={metrics.supplementCharactersWithoutSpaces.toLocaleString()} /><Info label="전체 표시 글자 수" value={metrics.charactersWithoutSpaces.toLocaleString()} /><Info label="한국어 글자 수" value={metrics.koreanCharacterCount.toLocaleString()} /><Info label="단어 단위" value={metrics.wordUnits.toLocaleString()} /><Info label="소제목 수" value={metrics.headingCount.toLocaleString()} /></dl></details></section>
    ...
    <section className="mt-6 rounded-[24px] border border-black/6 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">편집·미리보기</h2><p className="mt-1 text-sm text-[#77777f]">ContentDocument를 수정하고 현재 발행 플랫폼의 실제 Renderer 결과를 확인합니다.</p></div><div className="flex flex-wrap gap-2"><button aria-pressed={editorViewMode === "visual"} className="rounded-xl border px-4 py-2 text-sm font-semibold" onClick={() => setEditorViewMode("visual")} type="button">시각 편집</button>{activePlatform ? <><button aria-pressed={editorViewMode === "preview"} className="rounded-xl border px-4 py-2 text-sm font-semibold" onClick={() => setEditorViewMode("preview")} type="button">플랫폼 미리보기</button><button aria-pressed={editorViewMode === "html"} className="rounded-xl border px-4 py-2 text-sm font-semibold" onClick={() => setEditorViewMode("html")} type="button">HTML 소스</button></> : null}</div></div></section>
    {editorViewMode === "visual" && liveDocument ? <>{publicPostCatalogEnabled ? <section className="mt-4 rounded-[20px] border border-black/6 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">{catalogPlatform === "wordpress" ? "WordPress" : "티스토리"} 공개 게시글 동기화</h2><p className="mt-1 text-sm text-[#77777f]">{postCatalogState === "loading" ? `${catalogPlatform === "wordpress" ? "WordPress" : "티스토리"} 공개 게시글을 불러오는 중입니다.` : postCatalogMessage || "계정을 선택한 뒤 공개 게시글을 불러오세요."}{postsRetrievedAt ? ` · 마지막 동기화 ${formatReviewTime(postsRetrievedAt)}` : ""}</p></div><button className="rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50" disabled={!catalogConnection || postCatalogState === "loading"} onClick={() => void loadPostCandidates(true)} type="button">후보 새로고침</button></div>{postCatalogState === "session_expired" ? <p className="mt-2 text-sm text-amber-800">세션이 만료되었습니다. 플랫폼 연결에서 다시 연결해 주세요.</p> : null}{postCatalogState === "partial" ? <p className="mt-2 text-sm text-amber-800">일부 게시글만 불러왔습니다. 현재 후보는 사용할 수 있으며 새로고침할 수 있습니다.</p> : null}</section> : null}<ContentDocumentEditor candidates={publicPostCatalogEnabled ? postCandidates : []} disabled={working || operation !== "idle"} document={liveDocument} onChange={commitDocument} publishingPlatform={activePlatform} /></> : null}
    {activePlatform && editorViewMode === "preview" ? <section className="mt-4 rounded-[24px] border border-black/6 bg-white p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Bright Studio 구조 미리보기</h2><p className="mt-1 text-sm text-[#77777f]">실제 WordPress 테마에 따라 세부 모양이 달라질 수 있음</p></div><div className="flex gap-2"><button aria-pressed={previewMode === "desktop"} className="rounded-xl border px-3 py-2 text-sm font-semibold" onClick={() => setPreviewMode("desktop")} type="button">데스크톱</button><button aria-pressed={previewMode === "mobile"} className="rounded-xl border px-3 py-2 text-sm font-semibold" onClick={() => setPreviewMode("mobile")} type="button">모바일</button></div></div><div className={`mx-auto mt-4 overflow-hidden rounded-xl border bg-white transition-all ${previewMode === "mobile" ? "max-w-[390px]" : "max-w-full"}`}><iframe className="min-h-[720px] w-full bg-white" sandbox="" srcDoc={platformPreviewDocument(previewHtml, activePlatform, title)} title={`${activePlatform} Renderer preview`} /></div></section> : null}
    {activePlatform && editorViewMode === "html" ? <section className="mt-4 rounded-[24px] border border-black/6 bg-white p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">{activePlatform === "wordpress" ? "WordPress" : "Tistory"} HTML 소스</h2><p className="mt-1 text-sm text-[#77777f]">미리보기와 외부 Draft에 사용하는 읽기 전용 Renderer 결과입니다. HTML은 Canonical Source가 아닙니다.</p></div><button className="rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50" disabled={!previewHtml} onClick={() => void navigator.clipboard.writeText(previewHtml).then(() => setNotice("HTML을 클립보드에 복사했습니다."))} type="button">HTML 복사</button></div><pre className="mt-4 max-h-[560px] overflow-auto whitespace-pre-wrap rounded-xl bg-[#17171a] p-4 text-xs text-white">{previewHtml || "HTML을 생성하고 있습니다. 잠시 후 다시 확인해 주세요."}</pre></section> : null}
...
function ApprovalReadinessStatus({ review }: { review: NormalizedQualityReview }) {
  ...
  const overallLabel = readiness.applicationReady
    ? "신청 준비 기준 충족"
    : readiness.status === "blocked"
      ? "승인 준비 차단 항목 있음"
      : "승인 준비 검토 필요";

  const overallTone = readiness.applicationReady
  ...
  return (
    <section className="mt-5 rounded-[20px] border border-black/6 bg-[#fafafa] p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold">애드센스 승인 준비 상태</h3>
          <p className="mt-1 text-sm text-[#66666f]">
            원고 품질 승인과 사이트 전체의 AdSense 신청 준비 상태는 별도로 판단합니다.
          </p>
        </div>
        <span className={`rounded-full px-3 py-1 text-sm font-semibold ${overallTone}`}>
          {overallLabel}
        </span>
      </div>
          ...
          return (
            <article className="rounded-xl border border-black/6 bg-white p-4" key={check.key}>
              <div className="flex items-center justify-between gap-3">
                <strong>{labels[check.key] ?? check.key}</strong>
                <span className={`text-sm font-semibold ${statusTone}`}>
                  {statusLabel}
                </span>
              </div>

              <p className="mt-2 text-sm leading-6 text-[#66666f]">
                {check.message}
              </p>

              {check.action ? (
                <p className="mt-2 rounded-lg bg-[#f8f8fa] px-3 py-2 text-xs leading-5 text-[#55555d]">
                  다음 조치: {check.action}
                </p>
              ) : null}
            </article>
...
function StrategySummary({ content, document, quality, showTistoryDetails }: { content: UserContent; document: ContentDocument; quality: NormalizedQualityReview; showTistoryDetails: boolean }) {
  ...
  const strategyWarning = staleOpportunity
    ...
    : alignment?.status === "mismatch"
      ...
      : undefined;
  const reflected = [content.primaryKeyword, ...secondary].filter((keyword): keyword is string => Boolean(keyword && document.blocks.some((block) => {
  ...
  const diagnostic = metadata?.reviewDiagnostic ?? metadata?.generationDiagnostic;
  const sufficientElements = diagnostic?.requiredContentElements.filter((item) => item.status === "sufficient").length ?? 0;
  return (
    <section className={`mt-6 rounded-[20px] border bg-white p-5 ${strategyWarning ? "border-amber-300" : "border-black/6"}`}>
      ...
      <dl className="mt-4 grid gap-4 text-sm sm:grid-cols-3">
        ...
        {showTistoryDetails ? <Info label="티스토리 하단 태그" value={deriveContentTags(document, opportunity?.primaryKeyword ?? content.primaryKeyword).join(", ") || "생성되지 않음"} /> : null}
      ...
      <details className="mt-4 rounded-xl bg-[#f8f8fa] p-4">
        ...
        <dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Info label="정보 충분성" value={diagnostic ? `필수 요소 ${sufficientElements}/${diagnostic.requiredContentElements.length} 충분 · 위반 ${diagnostic.violations.length}개` : "진단 기록 없음"} />
          ...
          <Info label="품질" value={`${quality.overallScore}점 · ${quality.status === "ready" ? quality.approvalType === "exception" ? "예외 승인" : "품질 승인" : `개선 필요 ${quality.actionableTasks.length}개`}`} />
        </dl>
      </details>
    </section>
  );
