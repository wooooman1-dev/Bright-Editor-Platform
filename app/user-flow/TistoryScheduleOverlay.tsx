...
type ScheduleOutcome = "scheduled_verified" | "existing" | "scheduled_unverified";
...
export function TistoryScheduleOverlay() {
  ...
  useEffect(() => {
    if (!open || outcome || !context || !workspaceId || !connectionId || !scheduledAt) return;
  ...
  }, [connectionId, context, open, outcome, scheduledAt, workspaceId]);
      ...
      <section aria-labelledby="tistory-schedule-title" aria-modal="true" className="max-h-[92vh] w-full max-w-2xl overflow-auto rounded-[24px] bg-white p-6 shadow-2xl sm:p-7" role="dialog">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold" id="tistory-schedule-title">{outcome ? "예약 발행 처리 결과" : "예약 발행"}</h2>
            <p className="mt-2 text-sm leading-6 text-[#66666f]">{outcome ? "Tistory 예약 등록 처리 결과를 확인해 주세요." : "Bright Studio가 로컬에서 기다리지 않고 Tistory 자체 예약 기능에 현재 Revision을 등록합니다."}</p>
          </div>
        ...
        {outcome && outcomePresentation ? <div className="mt-6">
          ...
          <dl className="mt-5 grid gap-3 rounded-2xl bg-[#f8f8fa] p-4 text-sm sm:grid-cols-2">
            ...
            <Info label="Tistory 계정" value={selectedConnection?.displayName ?? connectionId} />
            <Info label="카테고리" value={content?.publishingPreparation?.tistory?.platformCategoryName ?? "카테고리 없음"} />
            <Info label="예약 시각" value={`${formatScheduleLocal(scheduledLocal)} · Asia/Seoul`} />
            <Info label="상태" value={outcomePresentation.statusLabel} />
          </dl>
...
function scheduleOutcome(status?: string): ScheduleOutcome {
  if (status === "scheduled_verified" || status === "existing") return status;
...
function scheduleOutcomePresentation(outcome: ScheduleOutcome) {
  if (outcome === "scheduled_verified") {
    return {
      eyebrow: "예약 등록 완료",
      title: "Tistory 예약 발행이 완료되었습니다.",
      description: "Tistory 예약 발행 등록과 외부 상태 확인이 완료되었습니다.",
      statusLabel: "외부 확인 완료",
      actionLabel: "완료",
  ...
  if (outcome === "existing") {
    return {
      ...
      description: "새 예약을 다시 실행하지 않았습니다. 기존 Tistory 예약 상태를 확인해 주세요.",
      statusLabel: "기존 예약 유지",
      actionLabel: "닫기",
  ...
  return {
    eyebrow: "외부 확인 필요",
    title: "Tistory 예약 상태를 직접 확인해 주세요.",
    description: "예약 등록 클릭은 완료됐지만 Tistory 외부 상태를 다시 확인해야 합니다. 자동 재시도하지 않습니다.",
    statusLabel: "외부 확인 필요",
    actionLabel: "닫기",
