...
function QualityStatus({ review }: { review: NormalizedQualityReview }) {
  ...
  const readyLabel = review.approvalType === "exception" ? "예외 승인 · 게시 준비 완료" : "품질 승인 · 게시 준비 완료";
  return <div className="mt-4 flex items-end gap-3"><strong className="text-4xl">{review.overallScore}</strong><span className={`mb-1 rounded-full px-3 py-1 text-sm font-semibold ${review.status === "ready" ? review.approvalType === "exception" ? "bg-blue-50 text-blue-800" : "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}>{review.status === "ready" ? readyLabel : "개선 필요"}</span></div>;
}
    ...
    <section className={`mt-6 rounded-[20px] border bg-white p-5 ${strategyWarning ? "border-amber-300" : "border-black/6"}`}>
      ...
      <details className="mt-4 rounded-xl bg-[#f8f8fa] p-4">
        ...
        <dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Info label="분량" value={`공백 제외 최소 4,800자 · 현재 ${metrics.charactersWithoutSpaces.toLocaleString()}자`} />
