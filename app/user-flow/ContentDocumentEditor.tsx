...
export function ContentDocumentEditor({
  ...
  return <section className="mx-auto mt-6 max-w-[900px] rounded-[24px] border border-black/6 bg-white px-6 py-8 shadow-[0_14px_50px_rgba(24,24,27,0.05)] sm:px-10 lg:px-14">
    ...
    <div className="mb-8 flex flex-wrap items-center justify-between gap-3">
      <div><h2 className="text-lg font-semibold">원고</h2><p className="mt-1 text-sm text-[#77777f]">본문을 클릭해 바로 수정하세요. 이미지에서는 프롬프트를 따로 수정하고 파일 또는 AI 생성 방식을 선택할 수 있습니다.</p></div>
    ...
    <div className="space-y-1">
      {document.blocks.map((block, index) => <article className={`group relative rounded-lg border border-transparent px-2 py-2 transition hover:border-black/8 hover:bg-[#fcfcfd] ${draggedId === block.id ? "border-[#ff6b6b] bg-[#fff7f7] opacity-60" : ""}`} draggable={!disabled} id={`editor-${block.id}`} key={block.id} onDragEnd={() => setDraggedId(undefined)} onDragOver={(event) => event.preventDefault()} onDragStart={() => setDraggedId(block.id)} onDrop={(event) => { event.preventDefault(); if (draggedId) void move(draggedId, index); }}>
        ...
        {block.type === "image" ? <ImageBlockEditor block={block} contentId={document.id} disabled={disabled} onChange={(next) => replace(block.id, next)} /> : null}
...
function blockLabel(block: ContentBlock) { if (block.type === "heading") return `H${block.level}`; if (block.type === "paragraph") return "문단"; if (block.type === "image") return block.source ? "이미지" : "추천 이미지"; if (block.type === "button") return purposeLabel(block.purpose ?? "cta"); return "비디오"; }
