...
export function ProjectApprovalSettingsCard({
  ...
  return (
    <>
      <section className="mt-6 rounded-[20px] border border-black/6 bg-white p-5" aria-labelledby="project-profile-title">
        <div>
          <h2 className="font-semibold" id="project-profile-title">Project 기본 프로필</h2>
          <p className="mt-1 text-sm leading-6 text-[#77777f]">Content Opportunity와 자동 Planning에서 사용하는 Project 문맥입니다.</p>
        </div>
        <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2">
          <div className="rounded-xl bg-[#f8f8fa] p-4">
            <dt className="text-[#77777f]">프로젝트 제목</dt>
            <dd className="mt-1 font-semibold">{project.name}</dd>
          </div>
          <div className="rounded-xl bg-[#f8f8fa] p-4">
            <dt className="text-[#77777f]">대표 주제</dt>
            <dd className="mt-1 font-semibold">{strategy.primaryTopic || "설정되지 않음"}</dd>
          </div>
          <div className="rounded-xl bg-[#f8f8fa] p-4 sm:col-span-2">
            <dt className="text-[#77777f]">프로젝트 설명</dt>
