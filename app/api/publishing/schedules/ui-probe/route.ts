...
import { TistoryScheduleUiProbeApplicationService, type TistoryScheduleUiProbeAuditRecord } from "../../../../application/publishing/TistoryScheduleUiProbeApplicationService";
...
export async function POST(request: Request) {
  try {
    ...
    const connection = await resolveConnection(workspaceId, body.connectionId, body.connectionName);
    ...
    const selectedTarget = await hasSelectedTarget(projectId, content, project, connection.id);
    const audits = {
      save: (record: TistoryScheduleUiProbeAuditRecord) => connectionStore.set("publishing-audits", record.operationId, record),
    };
    const result = await new TistoryScheduleUiProbeApplicationService(audits).execute({
      workspaceId,
      projectId,
      contentId,
      connection,
    ...
    return NextResponse.json({ result }, { status: result.status === "diagnosed" ? 200 : 400 });
  } catch (error) {
...
async function resolveConnection(
  ...
  if (!requestedName) throw new Error("Tistory connectionId 또는 정확한 계정 이름이 필요합니다.");
  const matches = (await connectionRepository.listByWorkspace(workspaceId)).filter((connection) => (
    connection.platform === "tistory" && connection.displayName === requestedName
  ));
  if (matches.length !== 1) throw new Error("현재 Workspace에서 정확히 일치하는 Tistory 계정 한 개를 찾지 못했습니다.");
  return matches[0];
}
...
function required(value: unknown): string {
  if (typeof value !== "string" || !value.trim()) throw new Error("Required schedule UI probe context is missing.");
  return value.trim();
}
