...
describe("QualityEngine dimension scoring", () => {
  ...
  it("scores a structured article higher than a short incomplete article", () => {
    ...
    // 표시 전용 구체성·떠넘김·근거활용·문체가 더해져 14개다 (D-050, 2026-08-29). 채점 대상은 여전히 8개.
    expect(complete.dimensions).toHaveLength(14);
  });
  ...
  it("keeps optional image and CTA diagnostics separate from approval state", () => {
    const base = structured();
