...
describe("VerificationTemporalPolicy", () => {
  ...
  it("derives temporal evidence from one bounded page context around a unique verified Claim excerpt", () => {
    ...
    expect(result).toMatchObject({ freshnessStatus: "fresh", effectiveFrom: "2026-01-01", effectiveUntil: "2026-12-31" });
    expect(result.temporalEvidence?.evidenceExcerpt).toContain(claimExcerpt);
  });

  it("does not expand ambiguous repeated Claim excerpts into temporal evidence", () => {
    const claimExcerpt = "지원 금액은 50만원입니다.";
    const pageText = `적용 기간은 2026-01-01부터 2026-12-31까지입니다. ${claimExcerpt} 과거 안내: ${claimExcerpt}`;
    const result = evaluateVerificationTemporalEvidence({
      requirement: { mode: "current" }, claimEvidenceExcerpt: claimExcerpt, pageText, claimValue: "50만원", observedAt: "2026-08-07T00:00:00.000Z",
    });
  ...
  it("treats explicitly time-independent Claims as freshness satisfied without inventing dates", () => {
    const result = evaluateVerificationTemporalEvidence({ requirement: { mode: "notRequired" }, pageText: "정적 사실", claimValue: "정적 사실" });
    expect(result).toMatchObject({ freshnessStatus: "fresh", fresh: true });
    expect(result.diagnostics).toContain("freshness_not_required");
  });
});
