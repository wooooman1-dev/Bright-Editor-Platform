...
const connectionMocks = vi.hoisted(() => ({ findById: vi.fn(), listByWorkspace: vi.fn() }));
...
vi.mock("../../../../../../app/application/publishing/TistoryScheduleUiProbeApplicationService", () => ({
  TistoryScheduleUiProbeApplicationService: class {
    execute = probeMocks.execute;
  },
}));

import { POST } from "../../../../../../app/api/publishing/schedules/ui-probe/route";
...
function request(body: Record<string, unknown>) {
  return new Request("http://localhost/api/publishing/schedules/ui-probe", {
    method: "POST",
...
describe("Tistory schedule UI probe API", () => {
  it("resolves one exact account name and passes server-owned context to the service", async () => {
    const response = await POST(request(validBody));
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body.result.status).toBe("diagnosed");
    expect(probeMocks.execute).toHaveBeenCalledWith(expect.objectContaining({
      workspaceId: "workspace-1",
      projectId: "project-1",
      contentId: "content-1",
      connection,
  ...
  it("rejects a non-Tistory or cross-Workspace account before the service runs", async () => {
    connectionMocks.findById.mockResolvedValue({ ...connection, platform: "wordpress" });

    const response = await POST(request({ ...validBody, connectionName: undefined, connectionId: "connection-1" }));
