...
const opportunity = confirmContentOpportunity(
  createContentOpportunityCandidate({
...
function approvalContent(
  overrides: Partial<UserData["contents"][number]> = {},
) {
  return {
    id: "content-1",
    workspaceId: "workspace-1",
...
describe("studio approval policy routes", () => {
  ...
  it("uses the Planning Content snapshot after the Project default changes", async () => {
    ...
    expect(requestInput(fetchSpy)).toContain(
      "Approval profile: Tistory · 비바레인 미술@1.0",
    );
    expect(requestInput(fetchSpy)).not.toContain("tistory_vivarain_art_v1");
  });

  it("uses the confirmed Content snapshot and complete Claim bundle for Generation", async () => {
    const officialUrl = "https://www.moma.org/collection/works/79802";
    const sourceEvidenceExcerpt =
      "The Starry Night is an official collection record at The Museum of Modern Art.";
    const artworkTitleExcerpt = "Artwork title: The Starry Night.";
    const creationYearExcerpt = "Date: 1889.";
    const mediumExcerpt = "Medium: Oil on canvas.";
    const dimensionsExcerpt = "Dimensions: 73.7 × 92.1 cm.";
    const institutionExcerpt =
      "Collection: The Museum of Modern Art, New York.";
    const claims = [
      {
        field: "artworkTitle",
        value: "The Starry Night",
        evidenceExcerpt: artworkTitleExcerpt,
      },
      {
        field: "creationYear",
        value: "1889",
        evidenceExcerpt: creationYearExcerpt,
      },
      {
        field: "medium",
        value: "Oil on canvas",
        evidenceExcerpt: mediumExcerpt,
      },
      {
        field: "dimensions",
        value: "73.7 × 92.1 cm",
        evidenceExcerpt: dimensionsExcerpt,
      },
      {
    ...
    const pageText = [
      ...
      dimensionsExcerpt,
      institutionExcerpt,
      "Official collection record and artwork details.",
    ].join(" ");
    ...
    const fetchSpy = vi.spyOn(globalThis, "fetch")
      ...
      .mockResolvedValueOnce(new Response(
        `<html><head><title>The Starry Night | MoMA</title><meta property="og:site_name" content="The Museum of Modern Art"></head><body>${pageText.repeat(6)}</body></html>`,
        {
          status: 200,
    ...
    expect(requestInput(fetchSpy, 2)).not.toContain(
  ...
  it("keeps the stable profile ID out of a canonical revision prompt", async () => {
    const document: ContentDocument = {
      ...
      metadata: {
        ...
        videoCount: 0,
        wordCount: 20,
        approvalPolicy: resolveApprovalPolicySnapshot(
          "adsense_approval",
          "tistory_vivarain_art_v1",
        )!,
      },
