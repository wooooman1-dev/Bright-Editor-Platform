...
describe("usable Content flow UI", () => {
  ...
  it("renders editor, quality review, exact preview controls, and isolated iframe path", () => {
    ...
    expect(html).toContain("품질 검토"); expect(html).toContain("검색 의도"); expect(html).toContain("정보 완성도"); expect(html).toContain("글자 수"); expect(html).toContain("티스토리 미리보기"); expect(html).toContain("HTML 보기"); expect(html).not.toContain("원본 HTML"); expect(html).toContain("원고"); expect(html).toContain("문서 구조 보기"); expect(html).toContain("끌어서 H2 이동"); expect(html).toContain("위로 이동"); expect(html).toContain("CTA 추가"); expect(html).toContain("수익 링크 추가"); expect(html).toContain("자동 추천 변경"); expect(html).toContain("Tistory 임시저장"); expect(html).toContain("Tistory 카테고리");
  });
