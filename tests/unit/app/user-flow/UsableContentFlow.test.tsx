...
describe("usable Content flow UI", () => {
  it("starts with a natural-language question and manual fallback", () => {
    ...
    expect(html).toContain("What would you like to create?"); expect(html).toContain("Continue manually");
  ...
  it("renders editor, quality review, exact preview controls, and isolated iframe path", () => {
    ...
    expect(html).toContain("품질 검토"); expect(html).toContain("검색 의도"); expect(html).toContain("정보 완성도"); expect(html).toContain("글자 수"); expect(html).toContain("Tistory Preview"); expect(html).toContain("Copy HTML"); expect(html).toContain("Tistory 임시저장"); expect(html).toContain("Tistory 카테고리");
  });
