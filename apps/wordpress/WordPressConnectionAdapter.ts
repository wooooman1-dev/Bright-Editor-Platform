...
export class WordPressConnectionAdapter {
  async verify(input: WordPressConnectionInput): Promise<WordPressPublicMetadata> {
    ...
    const authorization = createWordPressAuthorizationHeader(input.username, input.applicationPassword);
...
export function createWordPressAuthorizationHeader(username: string, applicationPassword: string): string {
