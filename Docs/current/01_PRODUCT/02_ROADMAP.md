...
이 절은 기존 Stage/Sprint/Release 설계를 삭제하거나 새 Stage를 만드는 변경이 아니다. 현재 Draft PR `#42`에서 실제 구현이 기존 Roadmap baseline보다 앞서 진행된 부분을 기록한다.

기존 표와 Stage 설명이 이 절의 실제 Repository 상태와 충돌할 경우, **현재 구현 상태를 판단할 때만 이 절과 Current Development Status를 우선**한다. Stage 전체 완료 여부와 장기 순서는 기존 Roadmap Gate를 유지한다.

## 29.1 Current repository context
...
## 29.2 Stage 5 / Sprint 7 foundation has advanced

The older Sprint 7 table entry `Design Approved, Not Implemented` is no longer sufficient as an implementation snapshot.

The full Sprint 7 remains Partially Implemented, but the current Repository contains an implemented and regression-protected Data Source / Opportunity Intelligence foundation including:

- Project-scoped DataSourceConnection ownership;
- separate credential reuse and resource Connection identity;
- current Google resource discovery on credential reuse;
- immutable established resource identity;
- Workspace-unassigned Connection projection;
- duplicate Project persistence protection and verified merge tooling;
- Project-scoped GSC/NAVER Evidence delivery to Planning;
- Opportunity Evidence confidence corrections;
- stale-result and cross-Project isolation guards.
...
## 29.3 Verification Claim / Source workstream

The Bright Finance Source Preflight incident introduced a Core-shared verification workstream that is now implemented and automated-verified.
...
## 29.4 Stage 6 / Sprint 8 WordPress implementation snapshot

The older Stage 6 `Planned` and Sprint 8 `Design Approved, Not Implemented` labels remain useful historical Roadmap baselines, but they no longer describe every component currently present in the branch.

Current Repository includes WordPress implementation foundation beyond the original primitive-only snapshot, plus shared Generated Claim verification consumption in publishing readiness.

This addendum does **not** mark Stage 6 or Sprint 8 complete.

Current Gate remains:

```text
latest Bright Finance live Provider verification
→ inspect generated HTML and verification metadata
→ WordPress Draft only after the content path is verified
→ external WordPress re-read verification
```

A real WordPress Draft using the latest Source/Verification correction has not been performed as part of this workstream. Public publish remains unauthorized.

## 29.5 Current priority while local external verification is unavailable

The highest-priority remaining Source/Verification Gate is intentionally small:
...
This harness does not execute WordPress Draft save or public publishing.

While the user's local environment is unavailable, documentation synchronization, deterministic regression coverage and architecture audit may continue. External verification must remain explicitly `Pending` until actual local/provider evidence exists.
