...
**2026-08-29 에 두 번째 방향으로 고쳤다. 코드 수정 완료, 앱 확인 남음.**

발행 기록의 `uploadedMedia` 를 `assetId` 로 찾아 이미 올라간 자산은 업로드하지
않는다 (`reusableMediaUrl`). 기록에 `sourceUrl` 이 없던 옛 건은 `readMedia` 로
되읽어 URL 을 얻고, 되읽기가 실패하면 — 사용자가 미디어를 지운 경우다 — 오류가
아니라 재업로드로 간다. 새로 저장하는 기록에는 `sourceUrl` 을 함께 남긴다.

canonical 문서에 URL 을 반영하는 첫 번째 방향은 택하지 않았다. 저장된 원고의
`source` 를 외부 URL 로 바꾸면 원고가 특정 발행 대상에 묶여, 다른 플랫폼 발행과
로컬 재편집이 함께 깨진다.
...
- **공개 Post 갱신 시 `draft_status` 검증 실패** — 확인했다. `updateDraft` 는
  공개 글을 초안으로 되돌리지 않으려고 `status` 를 일부러 안 보내는데, 검증은
  `expected.status ?? "draft"` 로 굳어 있었다. 갱신 직전에 읽은 그 글의 상태를
  기대값으로 쓴다. 공개 글이 초안으로 떨어지는 것은 여전히 잡는다.
  **코드 수정 완료, 앱 확인 남음.**
- **제목이 잘려 발행된 2편** — 코드 문제가 아니라 옛 데이터다. 제목 잘림은
  2026-08-20 02:13 UTC 커밋(「제목이 문장 중간에서 끊기지 않게 하고…」)으로
  고쳐졌고, 두 원고는 8/18·8/19 에 발행됐다. 워드프레스 제목은 온전하고 앱에
  저장된 제목만 잘려 있다. 그 두 원고를 갱신 발행할 때 제목을 복원해야 한다.
