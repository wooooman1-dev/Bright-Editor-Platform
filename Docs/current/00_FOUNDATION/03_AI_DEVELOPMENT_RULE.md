...
2. **코드 지도를 전부 읽는다** — 아래 목록. 코드를 만지기 전에 지형을 안다.
3. 저장소 루트 `todo.txt` 를 읽는다 — 남은 할 일과 지금 상태.
4. 커밋 상태를 확인한다 (3장의 명령).
...
| `02_ARCHITECTURE/17_PERSISTENCE_MAP.md` | 저장 구조, 세 겹 관문, 클라이언트·서버 병합 | 약 200줄 |
| `02_ARCHITECTURE/18_UI_FLOW_MAP.md` | 화면 5개, 전환, 화면별 API | 약 160줄 |

전부 읽어도 약 960줄이다. 이 비용은 오늘까지 지도가 없어서 생긴 잘못된 단정 한 번보다
싸다.

**지도가 없는 영역을 고쳐야 하면, 고치기 전에 그 영역을 읽고 지도를 남긴다.**
지도는 그렇게 늘린다. 아직 없는 영역: Tistory 발행, 연결·데이터소스.
...
| 프롬프트를 고쳤으니 모델이 그렇게 한다 | 스키마가 막으면 못 한다. 2026-08-29 본문 시각물을 프롬프트로 요청했는데 `structuredGenerationFormat` 이 `purpose: enum["hero"]`, `maxItems: 1`, `additionalProperties: false` 라 반환 자체가 불가능했다. **모델 출력 형식을 바꿀 때는 프롬프트와 스키마를 같이 본다** |
| 문제를 찾았으니 전부 오류다 | 아니다. 오류·빠진 검사·설계 공백·의도된 설계를 갈라서 말한다. 의도된 설계(D-050 의 weight 0, risk-based 근거 정책)를 내 판단으로 바꾸면 그건 수정이 아니라 정책 변경이다. 숫자로 영향을 세어 보고한 뒤 사장님이 정한다 |
...
| 데이터가 어디에 어떻게 저장되나 | `02_ARCHITECTURE/17_PERSISTENCE_MAP.md` |
| 화면이 몇 개이고 어떻게 도나 | `02_ARCHITECTURE/18_UI_FLOW_MAP.md` |
